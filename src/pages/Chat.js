...
async function llmCall(project, role, message, user) {
  try {
...
function Chat(props) {
  ...
  const getAnswer = async (message, role, project) => {
    ...
    const llmResponse = await llmCall(project, role, message, props.profile.id);
      ...
      {props.projects.projects && <VStack align='stretch' height='100%'>
        <Box h='calc(75vh)' bgColor='white' opacity='60%' p='5' borderRadius='2' borderWidth='1px' borderColor='dodgerblue'>
          <Code h='100%' p='4' w='calc(100%)' bgColor='E0E0E0' style={{ overflow: 'visible', overflowY: "scroll", height: "100%" }}>
            {
              messages.map((currentMessage, counter) => {
                return currentMessage.type === "question" ?
                  ...
                  <Box bgColor='CCC' p={3} fontSize="x-small" marginBottom={4}>
                    <ChatIcon boxSize={3} marginRight={2} />
                    <pre style={{ width: '100%', overflow: 'visible', overflowX: "scroll", overflow: 'scroll' }}>{currentMessage.content}</pre>
                  </Box>
              })
            }
          </Code>
        ...
        <Flex>
          <FormControl borderColor='none' padding='2px'>
            <Input w='calc(100%)' borderColor='dodgerblue' borderWidth='1px' opacity='60%' borderRadius='2px' textAlign='left' placeholder='Question...'
